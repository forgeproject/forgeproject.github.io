# Total Miner Files
### Maps • Mods • Lua Scripting • Tutorials • Resources

Source for the **ForgeProject Total Miner Files** site — maps, mods, a Lua scripting reference, and tutorials for the Total Miner community.

**Live site:** https://forgeproject.net

## What's inside

- [Maps](./maps.md) — ready-to-play worlds
- [Mods](./mods.md) — custom mods, built on the Lua scripting API
- [Mod Installation Guide](./mod-installation.md)
- [Lua Scripting Reference](./lua-docs.md) — interfaces, hooks, and examples
- [Tutorials](./tutorials.md) — Dig Deep, Survival, and general gameplay
- [FAQ](./faq.md)
- [Other Resources](./other.md)

## Quick Links

- **Official Total Miner Discord:** https://discord.gg/totalminer
- **Bug Reports:** https://github.com/forgeproject/forgeproject.github.io/issues
- **Feature Requests:** https://github.com/forgeproject/forgeproject.github.io/issues?q=is%3Aopen+is%3Aissue+label%3Aenhancement
- **Latest ZIP (main branch):** https://github.com/forgeproject/forgeproject.github.io/archive/refs/heads/main.zip

## Contributing

Have a map, mod, or script to share? Open an issue or submit a pull request. Special thanks to **B1ack W1dow** for ongoing development and community support.

## License

All content is provided for community use. Please credit the original creator when sharing or modifying files.
